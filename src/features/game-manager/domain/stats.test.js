...
test('play count includes partial sessions for that person and game', () => {
  const sessions = [
    ...
    session({
      state: 'complete',
      score: { mode: SCORE_ENTRY_MODES.PER_PLAYER, perPlayer: { ada: 40 } },
    }),
    session({
      state: 'complete',
...
test('personal best and average use only per-player numeric scores', () => {
  const sessions = [
    session({
      score: { mode: SCORE_ENTRY_MODES.PER_PLAYER, perPlayer: { ada: 40 } },
    }),
    session({
      score: { mode: SCORE_ENTRY_MODES.PER_PLAYER, perPlayer: { ada: 50 } },
    }),
    session({
      score: { mode: SCORE_ENTRY_MODES.SHARED, shared: 99 },
    }),
    session({
      score: { mode: SCORE_ENTRY_MODES.OUTCOME_MARKS, outcomes: { ada: 'win' } },
    }),
  ]
  assert.equal(personalBestForPersonAtGame(sessions, 'ada', gameKey), 50)
  assert.equal(averageScoreForPersonAtGame(sessions, 'ada', gameKey), 45)
})

test('points per minute requires timer export duration', () => {
  const withoutDuration = [
    session({
      score: { mode: SCORE_ENTRY_MODES.PER_PLAYER, perPlayer: { ada: 45 } },
    }),
  ]
