...
const core = createRtdbCore({ configuredBehavior: 'null', label: 'Game Manager Firebase' })
 ...
 * @returns {boolean}
 */
export function isGameManagerFirebaseConfigured() {
 ...
 */
export function getGameManagerAuth() {
 ...
 * @returns {() => void}
 */
export function subscribeGameManagerAuthState(onChange) {
 ...
 */
export async function signOutGameManager() {
  const auth = getGameManagerAuth()
  if (!auth) return
  await signOut(auth)
}
