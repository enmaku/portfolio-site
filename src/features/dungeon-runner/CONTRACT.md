...
- Optional `neuralRecoveryByModelId` on the **current match** blob records per-`modelId` **neural runtime recovery** coordinator state and **neural recovery terminal outcome** when exhaustion occurs during live play (including live-play **REFRESH** before refresh UX is shown), **finishing match**, or headless completion, so reload and resume surface the same blocking refresh or **setup** UX. **REFRESH** keeps the **current match** and defers headless completion until the player refreshes; **SETUP** clears the **current match** and restores **setup**.
