...
/** @typedef {{ status: 'ok', value: string } | { status: 'error' }} HumanWinRateTileResult */

/**
 * @typedef {object} HumanWinRateLoaderDeps
...
/**
 * @param {HumanWinRateLoaderDeps} [deps]
 * @returns {Promise<HumanWinRateTileResult>}
 */
export async function loadHumanWinRateTile(deps = {}) {
  try {
    ...
    const [numerator, denominator] = await Promise.all([
      countWhere('humanWon', true, queryDeps),
      countAll(queryDeps),
    ])
