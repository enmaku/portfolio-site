...
- **Game Manager ↔ Game Timer**: Separate **projects**; **timer leg** launches a **manager-linked timer** on the Game Timer surface (**launch config** in, compact **timer export** out via **game end** → **continue to scoring**). Owner same-browser handoff; **guests** are Timer-**room** only. Standalone **Game Timer** stays usable without signing in. Transport mechanics deferred; **stable host suffix preference** / join-link continuity required.
- **Game Manager → Catalog**: BoardGameGeek-backed **catalog** for collection and **game detail**; **custom collection entries** when search fails; **catalog attribution** required in UI. **Collection** ownership is required before a **play session** ([ADR 0026](./docs/adr/0026-game-manager-play-requires-collection-ownership.md); supersedes [ADR 0025](./docs/adr/0025-game-manager-collection-is-ownership-not-play-history.md)).
