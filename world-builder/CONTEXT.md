...
Persisted path geometry on the **world document** linking two **settlements** after a successful founding **expedition**—the march or sail that opened the daughter site. Each segment records its **expedition** mode (**land route** or **sail route**) and the connected **settlement** ids. Later increments may add segments from **trade route** activation and logistics pressure—not only founding marches.

_Avoid_: “Road” alone when both land and sail founding paths are meant; user-drawn paths in hands-off v1; decorative lines without simulation effect; erasing founding paths that did not result in a **settlement**.
