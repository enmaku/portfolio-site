...
- **Increment 3 diamond units**: resolved (updated 2026-07-19) — diamonds are integer whole-gem flows at 5,000 gp each, never fractional commodity quantities. Each gem uses 0.1 lb nominal cargo weight for capacity and transport. Claimed copper/silver/gold deposits yield multi-pound annual hauls sized to prosperity demand (not 1 lb token units); diamond deposits yield at least two whole gems per epoch. Settlements unable to justify a whole gem through remaining prosperity demand leave it for larger cities or off-map export.
