...
Umbrella for non-violent **factional** influence short of major war—commercial pull and neighbor **political pressure**—not **faction tax**, liege obligation, or **conquest**. Two separate passes under one concept (separate scoring and events; shared anti-churn / sticky-membership discipline). Rationale: [ADR 0022](../docs/adr/0022-world-builder-soft-power-two-passes.md).

1. **Trade agreement** (commercial): map-gray pins (**unaligned settlement** or singleton/**map-unaligned** **faction** capital) sustain dominant on-map trade with one living multi-control **faction**. Score the pin’s on-map trade **gold-piece value** with each **faction** (goods **bilateral obligations** to/from that banner’s taxed members and **trade partners**). Dominance arms for **faction** F when F holds a clear majority (**>50%**) **and** at least **2×** the next **faction**’s share (or the runner-up is ~0). Shorter streak → **faction territory overlay** paint + taxed-member trade-backed **rebellion** pressure; longer clear hold after paint → peaceful sticky join as a **trade partner** (**history log**). Pure **off-map trade** does not count.
2. **Political pressure** (neighbor influence): abutting / direct-corridor neighbor strength, wealth, shared **primary claim** borders, trade history, and related factors that push affiliation without modeling threats, identity, or backroom deals as separate engines—resolved after **trade agreement** scoring/joins and before major-war **conflict** in the same **epoch**. Successful pressure events are **alliances**:
   - Join / switch into an *existing* pressuring **faction** → subject becomes a **vassal** (weak external bind; never ordinary member or **trade partner** from pressure alone).
   - Neighboring **unaligned** / map-**unaligned** pins ally among themselves → mint a new peer **faction**: strongest / highest-tier involved pin is capital; every other involved pin joins as an ordinary full member (not **vassals**—they are co-founders of the new banner, not clients of an outside power).

Neither path enlists soft-controlled free towns as **conquest** allies—**faction tax** buys projected defense. Not whole-**faction absorption** from soft influence alone. If a commercial join extinguishes a singleton/**map-unaligned** **faction**, ordinary extinction / absorption-family **history log** applies—no empty banner, no nested living **faction**.

_Avoid_: “Trade alliance” / “alliance” as a second polity type beside **faction** (**alliance** names the pressure *event*, not a membership band); silent membership recolor from trade or pressure; treating the two soft-power passes as one blended score; conflating **alliance** with **trade partner** / **trade agreement**; counting off-map-only commerce as commercial dominance; inventing ally contributors for soft-power-controlled free towns; skipping the **history log** when soft affiliation becomes sticky; arming commercial dominance from plurality without majority or from a thin lead; single-**epoch** flicker without streak / clear-and-re-arm; sticky peaceful join on the same short streak that only meant to paint a free town; running **political pressure** after war in the same **epoch** (pressure is meant to amalgamate the board before force is needed); promoting joins-*into*-existing-banner pressure subjects to ordinary members without a later stronger event; seating peer-mint co-founders as **vassals** of the new capital.

### Trade partner

Sticky intra-**faction** membership band: a living **settlement** under a **faction**’s banner that does **not** pay **faction tax** and is **not** a **vassal** (no liege / **conditional loyalty** peel). Entry: trade-backed **rebellion** win toward the dominant trading **faction**, or peaceful sustained commercial **soft power** (**trade agreement**) join from a map-gray pin (**history log**)—including a singleton capital, whose prior **faction** extinguishes in that commit. Not an **alliance** / **political pressure** outcome—join-existing **alliances** enter as **vassals**; peer-mint **alliances** seat ordinary members under a new banner. Paints that **faction**’s color on the **faction territory overlay**; pin size matches ordinary member / **unaligned**. In **conquest** (as stake or contributor), defends or contributes **alone**—own **martial capacity** only; the banner does **not** add allied **projected might**. Exit (explicit events only): commercial dominance for that banner **clears and re-arms** for a sustained stretch → soft-**unaligned** (residual **soft power** paint may remain without membership); **conquest** by another **faction** → **vassal** of the victor; survival / corridor dependence on the host (or another) **faction** → upgrade/switch into taxed **vassal**; host **faction** extinction. No auto-promote to taxed member from high trade alone; no one-step peel into a rival’s **trade partner**—affiliation ends first, then rival commercial **soft power** / peaceful join may fire later. No armed **rebellion** from **trade partner** status—commercial peel (or **conquest**) only. Distinct from non-membership commercial paint and from taxed capital/member/**vassal** seats (those buy **faction** projection).

_Avoid_: Calling **trade partners** **vassals**; charging them **faction tax**; nesting them as a separate map **faction**; using **trade partner** for whole-**faction absorption** or for **alliance** / **political pressure** joins; granting them taxed-member defense while they skip the levy; silent commercial join or peel without a **history log** event; flickering affiliation from single-**epoch** trade noise; leaping from one banner’s **trade partner** to another’s in one event; staging **rebellion** for seats that already skip the levy.

### Political pressure

Soft-power neighbor-influence pass: sustained external push from abutting and/or corridor-linked **settlements** / **factions** without separate engines for identity, threat, or intrigue. Pressure factors (weights = tuning): neighbor **population** (size), wealth, **martial capacity** / local strength, shared-**primary claim** border count (multi-pin same-banner borders stack), on-map bilateral trade history with that neighbor/**faction**, and direct corridor boost (**land route**/road or **inland sail route**—not **open-sea**). Runs after **trade agreement** resolution and before major-war **conflict** in the same **epoch**. Rationale: [ADR 0022](../docs/adr/0022-world-builder-soft-power-two-passes.md). **Shared border** means exclusive **primary claim** cell adjacency between two living pins’ hinterlands—the same mask as **faction territory overlay** / **wealth overlay**—not geometric **haul-shed** circle overlap alone. Corridor multiplier: a direct built **land route**/road segment *or* a direct **inland sail route** (river/lake waterway) between the two pins—local neighbor traffic, not “news from abroad.” Explicitly excluded: **open-sea route** / ocean links and whole-network reach that is only open-sea-mediated. Not short-haul-without-corridor alone. Successful affiliation is an **alliance** event. Two shapes:

- **Join existing banner:** subject becomes a **vassal** of the pressuring **faction**—weak external bind; not ordinary member, not **trade partner**. Eligible subjects: **unaligned settlements**, map-**unaligned** / singleton-capital pins, and non-capital sticky members (**vassal**, ordinary member, **trade partner**) of another **faction**. Capitals of multi-settlement **factions** do not alliance-flip while they still have other living members—stripping hinterland pins leaves the capital as a map-**unaligned** singleton open to later **trade agreement**, **alliance**, or **conquest** over subsequent **epochs**.
- **Peer mint:** neighboring **unaligned** / map-**unaligned** pins ally among themselves → new peer **faction**; capital = strongest / highest-tier involved pin; other involved pins join as ordinary full members (co-founders—not **vassals**, not **trade partners**). Involved set = connected component of eligible free pins linked by **primary claim** adjacency and/or direct **road** / **inland sail** corridor. No town-tier readiness gate—any living free cluster of two or more may mint. Peer mint runs only when no existing-banner join clears the bar for those pins (**join-existing first**)—free towns crystallize together when no strong neighbor claimed them. When the active **faction** roster is already at the **12** cap, peer mint does not create an overflow banner: prefer join-existing; otherwise the cluster stays **unaligned** / map-**unaligned** until a slot frees.

Tempered so genuine independence can persist when pressure does not dominate: the subject’s own size/strength/wealth resists, and pressure from rival **factions** (or rival free-town clusters) splits the field—an **alliance** fires only when one side’s net pressure clearly leads (margin + sustained streak in the soft-power anti-churn family). Membership change is atomic within the **epoch**: by end of the pass the pin has either **alliance**-joined / switched or it has not—no intermediate “leaning” membership and no pressure-only pre-join **faction territory overlay** paint. Paint always shows end-of-**epoch** **factional control** / membership after politics resolves. Distinct from commercial **trade agreement** (**trade partner**) and from **conquest**.

_Avoid_: Blending into the commercial soft-power score; producing **trade partners** from pressure; seating join-existing subjects as ordinary members from pressure alone; seating peer-mint co-founders as **vassals**; silent banner recolor without an **alliance** **history log** event; treating every abutting free town as inevitable annexation; capital alliance-flip that extinguishes a multi-pin **faction** in one pressure event; inventing a separate collapse path beside map-**unaligned** singleton → later soft/war capture; counting geometric **haul-shed** overlap as a political border without **primary claim** contact; using corridors as the sole adjacency definition; treating whole-road or whole-river-component membership as equal to a direct corridor segment; letting **open-sea** / ocean links amplify **political pressure** (that is foreign news, not neighbor action); flipping on plurality without a clear lead over rivals or over the subject’s own resistance; single-**epoch** thrash without streak / clear-and-re-arm; preferring peer mint when an existing banner already dominates the pin; multi-stage pressure chrome that paints a banner before the **alliance** membership event fires.

### Alliance

Peaceful sticky-membership event from **political pressure**—not a **trade agreement**, not **conquest**, and not a second polity type. Join-existing shape: subject enters the pressuring **faction** as a **vassal**. Peer-mint shape: involved **unaligned** / map-**unaligned** pins crystallize a new **faction** (capital + ordinary members). Fires wholly within one **epoch** when armed (conditions may need a sustained streak to arm; the membership flip itself is not staged across epochs). Map chrome: green Material Symbols Outlined `partner_exchange` (diamond omitted) on affected pins for **one epoch** only (same temporal family as the recent-**conquest** mark); every involved pin shows the mark that **epoch**. No pre-join territory paint from pressure alone.

_Avoid_: “Alliance” as a standing membership band or map **faction**; conflating with **trade partner**; permanent handshake chrome; using alliance language for commercial soft-power joins; multi-epoch “joining in progress” that changes paint before membership.
...
- “The sandwiched free town felt **political pressure** from two abutting coastal pins plus a direct **road**—one **epoch** later the green handshake marked an **alliance**; it joined as a **vassal**, not a **trade partner**.”
- “Two river towns with an **inland sail route** between them peer-minted when no empire claimed them—handshake on both pins, ordinary members under the stronger capital; open-sea neighbors don’t get that corridor boost.”
- “The inland **faction** lost every border pin to coastal **alliances** over several **epochs**; the lonely capital went map-**unaligned** and fell next year—no same-**epoch** whole-banner absorb.”
...
- **Soft power / trade partner / political pressure** (this session): resolved — **Soft power** umbrella with two separate passes: **trade agreement** (commercial → paint then **trade partner**; existing behavior stays) and **political pressure** (→ **alliance**; after trade, before war; no pre-join paint). Join existing → **vassal**; peer mint → capital + ordinary full members (**join-existing first**; claim-adjacent and/or direct road/inland-sail cluster; no town-tier mint gate; roster-full → no overflow mint). Borders = **primary claim** adjacency; corridor = direct **road** or **inland sail** only (not **open-sea**). Factors: pop, wealth, **martial capacity**, border count, trade history, corridor (weights = tuning). Resistance = subject + rival split; streak arms; membership flip atomic per **epoch**. Handshake one **epoch**. Capitals of multi-pin **factions** don’t alliance-flip; capital-only leftover → map-**unaligned** → later capture. **History log** records **alliance**. Exact numeric thresholds remain implementation tuning. Rationale: [ADR 0021](../docs/adr/0021-world-builder-faction-territory-control-not-membership-only.md) (overlay control); [ADR 0022](../docs/adr/0022-world-builder-soft-power-two-passes.md) (two soft-power passes).
