...
Configuration during **colonization setup** for the founding wave. Pane ships in **colonization setup** (#391): **three-day haul distance** (scale calibration), **starting population**, **yield modifier** (marginal / typical / bountiful **arable envelope** interpretation), and **epoch batch** (years advanced per **epoch step**—default 50, configurable 1–100). Every field has a concrete default—sliders and controls are never indeterminate. **Begin colonization** enables once a valid **founding landing** is placed; unset-looking controls still carry defaults. Colonists are nameless and flavorless—no author-facing origin culture or homeland; the GM invents a past if they want one. Trade, diplomacy, and expansion temperament knobs wait for later increments. No author-facing RNG seed—colonization reuses **geography seed**. No **year cap** or auto-stop—the user keeps stepping **epochs** indefinitely.
