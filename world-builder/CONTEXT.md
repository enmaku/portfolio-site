...
Configuration during **colonization setup** for the founding wave. Pane ships in **colonization setup** (#391): **three-day haul distance** (scale calibration), **starting population**, **yield modifier** (marginal / typical / bountiful **arable envelope** interpretation), **epoch batch** (years advanced per **epoch step**—default 50, configurable 1–100), **land expedition range** (max trek length as a multiple of **three-day haul distance**—default **2×**, slider **1×–4×**), and **sail expedition range** (same basis for **sail expedition**—default **3×**, slider **2×–6×**; minimum stays above **land expedition range** minimum so maritime treks outrange overland at defaults). Every field has a concrete default—sliders and controls are never indeterminate. **Begin colonization** enables once a valid **founding landing** is placed; unset-looking controls still carry defaults. During `running`, only **epoch batch** remains editable mid-run; **land expedition range**, **sail expedition range**, and other colonist fields stay read-only after **begin colonization**. Colonists are nameless and flavorless—no author-facing origin culture or homeland; the GM invents a past if they want one. Trade, diplomacy, and expansion temperament knobs wait for later increments. No author-facing RNG seed—colonization reuses **geography seed**. No **year cap** or auto-stop—the user keeps stepping **epochs** indefinitely.
...
Persisted path geometry on the **world document** linking two **settlements** after a successful founding **expedition**—the **built corridor** between parent and daughter pins, not the bearing **expedition** trace. At founding the sim computes a terrain-following least-cost path (A→B) that may cross cells the trek never visited; **exploration fog** clears along that corridor. Each segment records its **expedition** mode (**land route** or **sail route**) and the connected **settlement** ids. Later increments may add segments from **trade route** activation and logistics pressure—not only founding marches.

_Avoid_: “Road” alone when both land and sail founding paths are meant; user-drawn paths in hands-off v1; decorative lines without simulation effect; erasing founding paths that did not result in a **settlement**; treating the marched **expedition** route as the persisted **route segment** geometry.

### Land route

**Route segment** from a successful **land expedition** founding. **Land route** cells apply a **movement cost** multiplier (faster/easier **haul** than open wilderness) for **travel time**, **haul-shed** reach, and overland routing.
...
**Route segment** from a successful **sail expedition** founding—a sailed path on the **Sail overlay** between coastal or river-mouth **settlements**. Does not apply a land **movement cost** multiplier; may seed increment 3 **trade route** / sea-lane candidates. **Sail route** cells are not **land route** cells.

_Avoid_: Conflating **sail route** with **trade route** (commodity activation waits for increment 3); drawing **sail route** through non-**Sail overlay** cells; expecting **sail route** to shorten overland **haul-shed** reach inland.

### Routes overlay

Colonization map layer (toggle like **exploration fog** and **Sail overlay**) showing persisted **route segments** from **successful founding** only—**land route** geometry in gray cobblestone and **sail route** geometry in cool cyan/teal, both drawn wide and nearly opaque so paths read clearly over biomes. Failed, rejected, or completed-without-founding **expeditions** do not appear here (**exploration fog** records where the realm has traveled). Read-only inspection of founding connectivity; not a separate persisted mask.

_Avoid_: “Roads overlay”; a single color for both modes; showing scouting paths that did not found; conflating with **trade route** activation state; treating the overlay as the authoritative visit map; **sail route** color that matches **Sail overlay** pink and becomes unreadable when both are on.
...
- **Annual epoch order**: resolved (epic cross-cut) — network (**expeditions**, founding, **roads**) → claim recompute → **survival triad** / **population collapse** → politics (latch, **factions**, **trade routes**, **conflict**). **Begin colonization** (#391) creates the founding **settlement** and tip without the survival leg; increment 1 (#392) applies **survival triad** once for **epoch** 0 without network/politics phases.
...
- **Routes and exploration**: resolved — [ADR 0012](../docs/adr/0012-world-builder-bearing-based-expedition-routing.md); terrain-aware **haul-shed** in increment 1 / shared colonization seam (#392 or prerequisite); increment 2 (#393) adds **route segment** persistence from founding **expeditions** (**land route** with **movement cost** multiplier; **sail route** without), toggleable **routes overlay** (gray cobblestone land, cyan/teal sail), fixed **land expedition** vs **sail expedition** at dispatch (coastal ~60/40 sail/land; inland land only; no portage in increment 2), sail founding limited to **Sail overlay**-reachable **logistics nodes**, sail steps keep shore within **6 cells**; **expeditions** use **random bearing** and **local terrain-following** steps (no pathfinding toward undiscovered targets); **expedition range** multipliers in **colonist settings** (defaults land **2×**, sail **3×**).
