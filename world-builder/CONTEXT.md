...
Colonization map layer—primary political game board in increment 3. A toggleable **overlay** of color-coded territory swathes (same presentation family as the **wealth overlay**)—not faction badges or chrome on settlement pins. Each living **faction** gets one distinct hue; fill covers exclusive **primary claim** cells of that **faction**’s living member **settlements** (same hinterland mask as **survival triad** / **population collapse** / **wealth overlay**)—not overlapping geometric **haul-shed** circles. Overlapping haul-sheds do not create shared political ownership; nearest-pin claim already assigns each cell to one **settlement**, hence one **faction** (or gray if that settlement is **unaligned**). **Control strength** sets opacity/intensity as a **hybrid**: discrete membership bands (capital / apex seat fullest; ordinary members high; **vassals** mid-band; **unaligned** neutral gray at a fixed readable opacity), with **conditional loyalty** logistics signals modulating within the **vassal** band so a march about to peel reads softer than a secure one—same dependence family as loyalty break (**grain circle**, **chokepoints**, corridors), not a separate politics stat. **Vassal** sites use the liege **faction**’s hue until defection. Visit-status alone does not paint territory. Thin claim perimeters may separate abutting hinterlands as on the **wealth overlay**. Rationale: [ADR 0018](../docs/adr/0018-world-builder-faction-territory-primary-claim.md).

_Avoid_: “Borders” as hand-drawn lines without simulation claims; painting territory independent of **history log**; pin-only faction badges or pin chrome instead of territory fill; claiming all visited cells as territory; capital-only fill that ignores **vassal** geography; painting overlapping geometric **haul-shed** circles as co-owned political land; inventing rivalry partners for **unaligned** gray sites; treating gray as a fake **faction** color in the legend; a continuous loyalty HUD unrelated to logistics dependence; flat equal opacity for capital and fragile **vassal**.
...
**Membership is sticky:** a **settlement** stays in its **faction** until an explicit political event—**vassal** defection, **maritime peel**, **faction absorption**, latch **fracture** into staggered mint queues, or **faction** extinction / **settlement** abandonment. Shifts in **logistics connectivity**, **trade routes**, or **haul-shed** overlap raise pressure toward those events; they do **not** silently re-partition **faction territory** each **epoch**.
...
**Expeditions**, automatic founding, **roads**, and **exploration fog** continue after latch—politics and exploration are concurrent layers, not a freeze. New **settlements** always join the origin **settlement**’s **faction** as **vassals** until **conditional loyalty** breaks—including long **open-sea expedition** foundings. Shared colonial kinship after a later split lives on the **realm** (and **legacy** **rivalry** causes), not as day-one independence. If the origin is temporarily **unaligned** (post-latch fracture waiting on staggered mint), the daughter starts **unaligned** with its origin cohort rather than inventing a banner early.

### Unaligned settlement

Living **settlement** with no current **faction** membership—short-term political independence inside the **realm**, not a map **faction** and not a **city-state**. Not a permanent third political tier beside **faction**. Useful **campaign kit** hook (“free town for now”). Kit and politics chrome list them under the **realm** as unaligned / free towns with crystallize-or-reabsorb risk—no **rivalry** edges. On the **faction territory overlay** they read as neutral gray fill, not a **faction** hue.

**Entry paths (baseline v1):** staggered post-latch wait before a component mints; daughters of still-**unaligned** origins; **soft independence**—a **conditional loyalty** break that would otherwise **spawn** a new **faction**, but the site is not yet **faction**-ready (below town-tier, or dependence/independence still ambiguous). Join-first defections (still corridor-dependent on a neighbor **faction**) still **join** that **faction** as **vassals**—they do not go **unaligned**. **Maritime peel**, clear town-tier independence spawns, war **faction absorption**, and dependence **faction absorption** still assign **faction** membership immediately. Catastrophe orphans (all town-tier+ members ruined while lesser pins live) are **out of v1**—existing capital-succession / extinction rules stay.
...
- **Conflict:** major-war **WOAC** outcome can force the loser into the winner the same way (**legacy** cause). Survivor = the victor.

Whichever trigger hits first may fire; heavy trade alone without A or B does **not** absorb—persistent commerce between separate **factions** can remain **rivalry** or ordinary exchange.
...
- **Faction territory overlay**: resolved — toggleable territory swathes (not pin chrome); one hue per **faction**; hybrid control strength (membership bands + loyalty logistics within **vassal** band); **vassals** under liege hue; **unaligned** = neutral gray; exclusive **primary claim** fill (not haul-shed overlap paint); not visit-status claims.
