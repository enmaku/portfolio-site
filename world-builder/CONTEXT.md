...
- **Faction territory overlay**: resolved — toggleable territory swathes (not pin chrome); one solid color per **faction**; **vassals** under liege color; **unaligned** = neutral gray; exclusive **primary claim** fill (not haul-shed overlap paint); not visit-status claims; no membership/loyalty shade or opacity bands.
