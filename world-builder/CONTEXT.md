...
- **Peer mint:** neighboring **unaligned** / map-**unaligned** pins ally among themselves → new peer **faction**; capital = strongest / highest-tier involved pin; other involved pins join as ordinary full members (co-founders—not **vassals**). Involved set = connected component of eligible free pins linked by **primary claim** adjacency and/or direct **road** / **inland sail** corridor. Requires at least one town-tier+ pin in the cluster (same readiness bar as lone-**unaligned** crystallize)—all-hamlet clusters do not mint; they wait or fall to join-existing. Peer mint runs only when no existing-banner join clears the bar for those pins (**join-existing first**)—free towns crystallize together when no strong neighbor claimed them. When the active **faction** roster is already at the **12** cap, peer mint does not create an overflow banner: prefer join-existing; otherwise the cluster stays **unaligned** / map-**unaligned** until a slot frees.
