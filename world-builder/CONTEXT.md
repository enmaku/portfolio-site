...
- **Colonization running chrome**: left panel → **Reset colonization** (red) then **realm economy**; right panel → **Next epoch** (green) then **validation advisory** (only at **epoch** 0) plus **sim status** census from commit onward; increment 3 adds **event feed**. **Campaign kit** export follows the same right-panel forward pattern when it ships. Terrain generation controls and editable **colonist settings** stay out of `running` chrome; map overlay toggles remain for inspect.
