...
Colonization map layer showing the candidate **trade route** graph: dormant candidates draw faintly, edges with positive current-**epoch** flow draw strongly, and ordinary overland, road, inland-water, and open-sea modes use distinct colors. Blocked or embargoed corridor styling is deferred to the politics pass that implements those blocks.

_Avoid_: Decorative path lines without commodity/volume semantics; user-drawn routes in hands-off mode; shipping embargo chrome before politics can create blocks; hiding geography-proposed candidates until they carry flow.

### Wealth overlay

Colonization heatmap of each settlement’s combined financial position: **settlement trade account** balance plus any nonnegative **external trade account** claim, normalized by the same projected annual income used for the **credit limit**. Positive years-of-income appear green, debt appears red, and accounts near balance remain neutral; raw **gold-piece value** remains available on settlement inspect. When projected income is zero, any positive balance saturates green, any debt saturates red, and a true zero stays neutral. The overlay is toggleable like other simulation layers.

_Avoid_: Raw gp intensity that makes large cities dominate by scale alone; inventing fake income to avoid saturation; a second income formula that diverges from the **credit limit**; showing annual **material prosperity** instead of the balance sheet; color without a signed legend; interpreting green wealth as physical coin stored on the map.

### Settlement trade tooltip

Hover detail on a **settlement** pin showing its current realm **settlement trade account** balance and listing every simulated commodity’s current-**epoch** trade role. **Port settlements** also show their separate **off-map trade** credit. For each commodity, show the connected market’s **local price** beside a green upward arrow for exported, a red downward arrow for imported, both arrows when both occurred, and a neutrally colored dash for neither. Icons accompany color so direction remains readable without color alone.

_Avoid_: “Cash” when no physical coin is simulated; reducing gross two-way trade to one net arrow; prose summaries instead of a compact commodity list; omitting commodities with no trade; using color as the only directional cue; presenting local production as an export when no goods moved; omitting **local price** from the commodity row.
...
- **Increment 3 overlays**: resolved (updated 2026-07-15) — **faction territory** = member pins + geometric **haul-shed** circle fill (**vassals** under liege; contested overlap treatment); visit-status alone does not claim; exclusive calorie-claim cells are not the territory fill. **Trade route overlay** shows dormant candidates faintly and active flow strongly, with distinct colors for ordinary overland, road, inland water, and open sea; embargo/block chrome waits for the politics pass. **Wealth overlay** maps combined realm balance plus off-map claim, normalized by the same projected annual income as the **credit limit**: reserves green, debt red; zero projected income saturates signed color instead of dividing by zero. **Settlement trade tooltips** itemize realm balance and, for ports, separate off-map credit, then show every commodity with **local price** and icon-plus-color export/import/both/neither state. **Settlement trade profiles** keep structural pre-trade surplus/deficit for campaign-kit copy. **Strategic resource** layers reuse terrain; **rivalry** heat remains deferred to inspect/debug.
