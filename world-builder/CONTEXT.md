...
Who the **faction territory overlay** attributes a living **settlement**’s **primary claim** to. Broader than sticky membership: includes capital / member / **vassal** / **trade partner** seats of a **faction**, and map-gray pins under that **faction**’s sustained **soft power**. Membership changes only on explicit events; **soft power** control may shift with trade dominance under anti-churn without rewriting banners.

_Avoid_: Equating control with membership; treating overlay hue as proof of **faction tax** or **rivalry** actor status.

### Soft power

Non-membership **factional control** earned when a map-gray pin (**unaligned settlement** or singleton/**map-unaligned** **faction** capital) sustains dominant on-map trade with one living multi-control **faction** over rivals—commercial pull, not **faction tax** or liege obligation. Raises pressure toward trade-backed **rebellion** (for taxed members) and paints the dominant **faction**’s hue on the **faction territory overlay** while the pin stays politically free. Not whole-**faction absorption** from commerce alone.

_Avoid_: “Trade alliance” as a second polity type; silent membership recolor from trade volume; **soft power** over pins that already hold sticky membership in another **faction** (membership wins); counting off-map-only commerce as on-map dominance without a stated rule.

### Trade partner

Sticky intra-**faction** membership band: a living **settlement** under a **faction**’s banner that does **not** pay **faction tax** and is **not** a **vassal** (no liege / **conditional loyalty** peel). Default destination when a trade-backed **rebellion** succeeds toward the dominant trading **faction**. Paints that **faction**’s color on the **faction territory overlay**; pin size matches ordinary member / **unaligned**. Distinct from **soft power** (no membership) and from taxed capital/member/**vassal** seats.

_Avoid_: Calling **trade partners** **vassals**; charging them **faction tax**; nesting them as a separate map **faction**; using **trade partner** for whole-**faction absorption**.
