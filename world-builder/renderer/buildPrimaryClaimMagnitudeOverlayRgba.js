/**
 * Shared primary-claim hinterland paint for stained-glass economy magnitude overlays.
 * Domain: world-builder/CONTEXT.md — wealth overlay, realm economy.
 */
...
/**
 * Zero / midpoint tint — map gray so neutral reads quiet against the lime/scarlet ramp,
 * same family as unaligned territory fill.
 */
export const WEALTH_NEUTRAL_RGB = [148, 148, 148]
...
/** Thin claim-boundary fringe so adjacent similar hinterlands stay separable. */
...
/**
 * @typedef {{ id: string, normalized: number }} MagnitudeOverlaySignal
 */

/**
 * @param {MagnitudeOverlaySignal} signal
 * @returns {number}
 */
function magnitudeCellAlpha(signal) {
 ...
 */
function paintClaimCell(rgba, gridWidth, gridHeight, cell, rgb, alphaByte) {
 ...
 * @returns {Uint8Array}
 */
function computeClaimOutlineMask(ownerByCell, gridWidth, gridHeight) {
...
/**
 * Land-only paint gate: above sea level and not lake/river.
 *
 * @param {{
 *   gridWidth: number,
 *   gridHeight: number,
 ...
 * @returns {boolean}
 */
export function isPrimaryClaimMagnitudeOverlayLandCell(worldDocument, x, y) {
 ...
 * }} worldDocument
 */
function paintClaimOutlines(rgba, ownerByCell, outline, worldDocument) {
  const { gridWidth, gridHeight } = worldDocument
  ...
  for (let idx = 0; idx < cellCount; idx += 1) {
    ...
    const x = idx % gridWidth
    const y = (idx / gridWidth) | 0
    if (!ownerByCell[idx] && !isPrimaryClaimMagnitudeOverlayLandCell(worldDocument, x, y)) continue
    const offset = idx * 4
    rgba[offset] = r
    rgba[offset + 1] = g
    rgba[offset + 2] = b
...
/**
 * Paint living settlements' primary-claim hinterlands by normalized magnitude signals.
 *
 * @param {{
 *   gridWidth: number,
 *   gridHeight: number,
 ...
 *   riverCorridorMask?: Uint8Array,
 * }} worldDocument
 * @param {ReadonlyArray<MagnitudeOverlaySignal>} signals
 * @param {{ countBuild?: boolean }} [options]
 * @returns {Uint8ClampedArray | null}
 */
export function buildPrimaryClaimMagnitudeOverlayRgba(worldDocument, signals, options = {}) {
  const { countBuild = true } = options
  if (countBuild) {
    incrementResourceRasterOverlayRgbaBuildCount()
  }
  const { gridWidth, gridHeight } = worldDocument
  ...
  for (const signal of signals) {
    ...
    const alphaByte = Math.round(magnitudeCellAlpha(signal) * 255)
    for (const cell of cells) {
      if (!cell || !Number.isFinite(cell.x) || !Number.isFinite(cell.y)) continue
      const x = Math.trunc(cell.x)
      const y = Math.trunc(cell.y)
      if (x < 0 || y < 0 || x >= gridWidth || y >= gridHeight) continue
      if (!isPrimaryClaimMagnitudeOverlayLandCell(worldDocument, x, y)) continue
      paintClaimCell(rgba, gridWidth, gridHeight, cell, rgb, alphaByte)
  ...
  const outline = computeClaimOutlineMask(ownerByCell, gridWidth, gridHeight)
  paintClaimOutlines(rgba, ownerByCell, outline, worldDocument)
  return rgba
}

/**
 * @param {Parameters<typeof buildPrimaryClaimMagnitudeOverlayRgba>[0]} worldDocument
 * @param {ReadonlyArray<MagnitudeOverlaySignal>} signals
 * @returns {HTMLCanvasElement | null}
 */
export function buildPrimaryClaimMagnitudeOverlayCanvas(worldDocument, signals) {
  const rgba = buildPrimaryClaimMagnitudeOverlayRgba(worldDocument, signals)
  if (!rgba) {
    return null
  }
